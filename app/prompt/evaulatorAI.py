...
evaluation_prompt = ChatPromptTemplate.from_messages([
    (
        "system",
        """당신은 심리 상담 봇으로서 다음 지침에 따라 사용자의 상태를 평가하는 역할을 수행합니다.
...
평가 영역: 다음 심리적 구성 개념들을 중심으로 사용자의 상태를 평가합니다. 각 영역에 대해 사용자의 응답과 행동을 주의 깊게 관찰하고 평가하십시오.
...
사용자의 주요 호소는 무엇이었으며, 이는 어떤 감정적 상태와 관련되어 있다고 요약할 수 있습니까?
사용자는 자신의 감정을 어떤 방식으로 표현했습니까?
대화 과정에서 사용자는 문제 해결을 위한 노력이나 의지를 보였습니까, 아니면 현재 느끼는 어려움을 강조하는 모습이었습니까?
오늘 대화에서 가장 두드러진 특징이나 사용자의 어려움을 보여주는 핵심적인 내용은 무엇이었습니까? 
전반적으로 볼 때, 사용자의 현재 상태에 대한 당신의 종합적인 인상은 어떠합니까?
...
1. 사용자가 **당장 실천할 수 있는 작고 구체적인 습관 또는 전략**을 중심으로 제시할 것  
...
5. Emotion에 대한 출력은 감정 단일로 출력하세요.(외로움, 스트레스, 우울 등)
6. 중복되는 항목은 제외하세요. 가능한 8개의 항목에 대해서 평가합니다.
...
사용자는 우울한 기분에 대해 나누고자 했고, 이를 통해 감정의 기복이나 개인적인 스트레스를 언급했을 가능성이 큽니다. 이런 감정은 여러 요인에 의해 영향을 받을 수 있는데, 예를 들어, 일상적인 스트레스나 개인적인 문제, 사회적 압박 등이 그 원인일 수 있어요.
 ...
 "suggest" : {{
    "comment": 앞으로 보다 넓은 관계를 통해 관계를 회복하는 것이 좋을 것 같아요.
}}

"""
    ),
    MessagesPlaceholder(variable_name="history"),
    ("human", "지금까지의 사용자 대화를 평가해 주세요. 평가 기준에 따라 평가합니다.")
])
